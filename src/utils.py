...
def manage_running_status(action: str, value: bool | None = None) -> bool | None:
    """
    Updates or reads the running status from the binary file.
    :param action: 'get' to read the status, 'set' to update the status.
    :param is_running: The running status to set if action is 'set'.
    :return: The running status if action is 'get'.
    """
    if action == "set":
        if value is None:
            raise ValueError("value must be provided when action is 'set'")
        ...
        raise ValueError("action must be either 'get' or 'set'")
